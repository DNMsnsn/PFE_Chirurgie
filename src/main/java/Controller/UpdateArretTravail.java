...
@WebServlet("/UpdateArretTravail")
public class UpdateArretTravail extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int id = Integer.parseInt(request.getParameter("idarret"));
		...
		try {
			...
			if(idPatient != 0 && idMedecin != 0) {
				...
				if(dao.update(arr)) {
					...
					hs.setType_histo("UPDATE_Arret_Travail");
					...
					request.setAttribute("error", "Arret de Travail non Modifier !");
